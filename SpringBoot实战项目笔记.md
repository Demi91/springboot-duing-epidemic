...
整合一个大的页面的问题：
1） 代码合并到一个页面中，请求合并为一笔请求，当数据复杂后，可以使用多笔请求——推荐
2） A.html ->   B.html 嵌入其中  -> C.html  ->  D.html



cookie和session
1) cookie在客户端  session在服务端
2) cookie不安全，可以模拟，session相对安全，往往保存在缓存中
3) cookie有不可跨域名的特性，有大小限制，最大 4k



分布式session
同一个服务  部署到多台机器   机器A   机器B   机器C 
当登录请求  分发到 机器A后   如何让session在机器B和机器C中也生效
...
3)  session复制，当任一机器的session发生变化，广播给其他所有节点，以保证同步
...
Spring  Security 和 Shiro  权限管理框架
...
AOP  面向切面编程    主要解决非业务的逻辑问题，比如日志、事务等等
